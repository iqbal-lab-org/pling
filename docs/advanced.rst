...
**Network thresholds:** ``--containment_distance`` and ``--dcj`` control the thresholds at which edges are added to the containment and DCJ-Indel networks. If looking for recent transmissions, we recommend trying threshold 0.3 for the containment distance. If you want to try different DCJ-Indel thresholds with the same containment threshold, you can run pling at different DCJ-Indel thresholds with the same output directory -- this will mean that only the DCJ-Indel network will be calculated anew, which will significantly reduce runtime. Changing the containment threshold will prompt pling to rerun the whole workflow though, and so you should change the output directory if you don't want to lose previous results.
...
**Clustering parameters:** ``--bh_connectivity`` and ``--bh_neighbours_edge_density`` both determine how a hub plasmid is defined. ``--bh_connectivity`` determines to how many plasmids a hub should at least be connected to, while ``--bh_neighbours_edge_density`` sets the maximum edge density allowed between a hub plasmid's neighbours before the plasmid is no longer labelled as a hub. ``--small_subcommunity_size_threshold`` controls the size up to which small subcommunities may be joined to neighbouring larger subcommunities.

**Visualisation styles:** As of v3, by default only visualisations of results from ``dcj_thresh_4_graph`` are output, but with ``--visualisation`` you can opt to output no visualisations, or all possible visualisations (this option will perform equivalently to pling v2). With ``--output_type`` you can opt to output Cytoscape-compatible json files together with or instead of the default html visualisations. The json files can be loaded directly into Cytoscape, and used in python together with networkx. There is also a Cytoscape style file available, which provides a style similar to the default html visualisations. It can be downloaded from here: https://raw.githubusercontent.com/leoisl/plasnet/main/plasnet/ext/templates/plasnet_style.xml. You can also add additional metadata to node names with ``--plasmid_metadata``, e.g. Inc types. Generated HTML visualisations are checked after each run; any file that uses Cytoscape but lacks a Cytoscape.js source tag is repaired to use a nearby ``libs/cytoscape.min.js`` file when available, or a CDN source otherwise.

**ILP solver:** Calculating the DCJ-Indel distances involves solving an integer linear problem (ILP), and pling allows a choice between two ILP solvers for this: GLPK or Gurobi. GLPK is free and bundled with pling, but a bit slower. Gurobi is commercial software, with a free academic license, and you must have a valid license and the ``gurobipy`` Python package installed beforehand to run pling with it. Both solvers output the same final result. Generally calculating DCJ-Indel is an NP-hard problem, which means in its worst case calculation will take a very long time. The ``--timelimit`` variable sets a time limit for how long the ILP solver takes with a pair, before giving up and outputting the most optimal result it has at that point. However our experience is that when running with integers from alignment, the DCJ-Indel calculation is very quick.

**Snakemake arguments:** Arguments ``--cores``, ``--profile`` and ``--forceall`` are passed directly to Snakemake. Please refer to Snakemake documentation (https://snakemake.readthedocs.io/en/v7.0.0/) for further information. Through ``--resources`` you can pass a path to a ``resources.tsv`` file, which will define number of threads and memory allocated for each rule in pling's Snakemake workflows. The format should be the same as the file found under ``pling/resources.tsv``. If you use more than one thread in any rule, remember to set ``--cores`` to the maximum number of threads you'd like to use.

**Varying thresholds:** With ``--reuse_previous`` you can give a path to a previous clustering, and get a new clustering with different thresholds without recomputing all distances. This works if the new containment threshold you choose is less than or equal to the previous threshold you ran pling with; therefore, if you are going to experiment with thresholds, make sure to start with the highest containment distance you will be considering (e.g. 0.5, the default).
...
As of v3, when providing your own integerisation you can now construct the containment network just using containment distances computed from sourmash results, skipping alignment entirely, using the flag ``--sourmash_only``. The threshold given by ``--containment_distance`` is used for the containment network construction.

The entries in a unimog file are formatted similar to fasta files - the first line begins with ">" and the entry name, and the second lines contains the integer sequence. In the sequence, integers are separated by a space, and negative integers represent reverse complements. The end of a sequence is indicated by either ")" or "|", where ")" means the genome is circular, and "|" means the genome is linear. Here's an example of what it can look like:
...
.. code-block:: console
	...
	pling add GENOMES_LIST OUTPUT_DIR PREVIOUS_PLING...
...
``pling submatrix OUTPUT_DIR`` outputs DCJ-Indel distances as matrices for each subcommunity, and can also calculate neighbour-joining trees. The main options are ``--submatrices_dir`` to choose the output directory, ``--ignore_containment`` to calculate missing DCJ-Indel distances for pairs that did not meet the containment threshold previously, and ``--vis_trees`` to plot the neighbour-joining trees. It also accepts the same Snakemake resource options as the clustering commands, including ``--batch_size``, ``--ilp_solver``, ``--timelimit``, ``--resources``, ``--cores``, ``--profile`` and ``--forceall``.
...
4. **Use Gurobi as the ILP solver:** This is not worth trying unless you have already tried the previous options and still want a quicker run time. By default, pling calculates DCJ-Indel with the ILP solver GLPK, which is free and open source. Gurobi is an alternative, faster ILP solver, but is commercial software. Please see installation of optional dependencies for more instructions on how to set it up. This is only worth doing if you are planning to use pling repeatedly on large datasets. After installation, use ``--ilp_solver gurobi`` to run pling with Gurobi.

Depreciated: Integerisation from annotation (pling v1)
------------------------------------------------------

As of version 2, pling no longer does integerisation from gene annotation, as we found it generally unreliable for a variety of reasons. This functionality is not present in the current ``pling cluster`` command line interface. If you are still interested in getting DCJ-Indel distances and clustering from genes rather than pairwise synteny blocks, you can still use version 1. You can download that version from https://github.com/iqbal-lab-org/pling/releases/tag/v1.1.1 , and you will need the following dependencies:
